class Vanagon
  class Platform
    class Smartos < Vanagon::Platform
      # The specific bits used to generate a smartos package for a given project
      #
      # @param project [Vanagon::Project] project to build a smartos package of
      # @return [Array] list of commands required to build a smartos package for the given project from a tarball
    ...
    #   end
      ...
      # Method to transform a standard version into the format expected by pkgsrc packages
      def pkgsrc_version(version, release)
      end
      ...
      def initialize(name)
        ...
        @patch = "/opt/local/bin/gpatch"
        # @sed = "/usr/gnu/bin/sed"
        @sed = "/opt/local/bin/gsed"
        # @num_cores = "/usr/bin/kstat cpu_info | /usr/bin/ggrep -E '[[:space:]]+core_id[[:space:]]' | wc -l"
        @num_cores = "/usr/bin/kstat cpu_info | /opt/local/bin/ggrep -E '[[:space:]]+core_id[[:space:]]' | wc -l"
        # @num_cores = "/usr/bin/kstat cpu_info | /usr/bin/grep -E '[[:space:]]+core_id[[:space:]]' | wc -l"
        super
        @architecture == "x86_64"
          @platform_triple = "x86_64-pc-smartos.#{@os_version}"
