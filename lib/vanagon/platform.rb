  ...
  class Platform
    ...
    def generate_compiled_archive(project)
      ...
      [
        ...
        "cd #{archive_directory}/#{name_and_version}; #{bill_of_materials_command}; #{tar} cf ../../#{name_and_version_and_platform}.tar --sort=name --pax-option='delete=atime,delete=ctime' --clamp-mtime --mtime='#{Time.at(project.source_date_epoch).strftime('%Y-%m-%dT%H:%M:%SZ')}' *",
        "gzip --no-name -9c #{name_and_version_and_platform}.tar > #{name_and_version_and_platform}.tar.gz",
